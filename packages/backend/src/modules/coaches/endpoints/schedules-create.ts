...
// COACHING-V1 — a club owner/admin posts a recurring lesson slot. See modules/coaches/CoachScheduleService.ts.
export const meta = {
	tags: ['coaches'],
...
export const paramDef = {
	...
	properties: {
		...
		sport: { type: 'string', maxLength: 32 },
		weekday: { type: 'integer', minimum: 1, maximum: 7 },
		startTime: { type: 'string', maxLength: 5 },
		durationMinutes: { type: 'integer', minimum: 15, maximum: 1440 },
		timezone: { type: 'string', maxLength: 64 },
		venueId: { type: 'string', nullable: true }, venueName: { type: 'string', nullable: true, maxLength: 256 }, venueAddress: { type: 'string', nullable: true, maxLength: 512 },
		lat: { type: 'number', nullable: true }, lng: { type: 'number', nullable: true },
		capacity: { type: 'integer', minimum: 1, maximum: 50 },
		bookingMode: { type: 'string', enum: ['single', 'series', 'pack', 'several'] },
		packSize: { type: 'integer', nullable: true, minimum: 2, maximum: 52 },
		priceTiers: { type: 'array', items: priceBand, maxItems: 3 },
		cancellationPolicy: { type: 'object', properties: { windowHours: { type: 'integer', minimum: 0, maximum: 336 } } },
		...
		minLevel: { type: 'number', nullable: true }, maxLevel: { type: 'number', nullable: true },
		gender: { type: 'string', enum: ['any', 'coed', 'female', 'male'] },
		ageGroup: { type: 'string', enum: ['any', 'junior', 'adult', 'senior'] },
		publishLeadHours: { type: 'integer', minimum: 1, maximum: 672 },
		status: { type: 'string', enum: ['active', 'paused'] },
		tagIds: { type: 'array', items: { type: 'string' }, maxItems: 20 },
	...
	constructor(private coachScheduleService: CoachScheduleService) {
		super(meta, paramDef, async (ps, me) => {
			try {
				...
				for (const k of KEYS) if ((ps as Record<string, unknown>)[k] !== undefined) data[k] = (ps as Record<string, unknown>)[k];
				const s = await this.coachScheduleService.create(me, ps.channelId, data as CoachSchedulePatch & { name: string; weekday: number; startTime: string });
				return await this.coachScheduleService.packSchedule(s, me);
