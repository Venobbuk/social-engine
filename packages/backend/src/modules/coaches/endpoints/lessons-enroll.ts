...
export const paramDef = { type: 'object', properties: { scheduleId: { type: 'string', format: 'misskey:id' }, mode: { type: 'string', enum: ['series', 'pack'] } }, required: ['scheduleId', 'mode'] } as const;
	...
	constructor(private coachScheduleService: CoachScheduleService) {
		super(meta, paramDef, async (ps, me) => {
			try {
				...
				const { enrollment, booked } = await this.coachScheduleService.enroll(s, me, ps.mode);
