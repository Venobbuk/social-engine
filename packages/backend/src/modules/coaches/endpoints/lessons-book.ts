...
import { coachErrors } from './_shared.js';
...
export const paramDef = { type: 'object', properties: { meetId: { type: 'string', format: 'misskey:id' }, accessToken: { type: 'string', nullable: true } }, required: ['meetId'] } as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			try {
				const p = await this.coachScheduleService.bookSingle(meet, me, ps.accessToken ?? null);
