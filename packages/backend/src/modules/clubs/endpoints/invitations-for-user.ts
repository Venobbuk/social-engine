...
// CLUB-INVITE-V1: a player's page "Invite to a club" — the clubs I own / admin, each with that player's status
// (member | invited | requested | declined | none).
export const meta = {
	tags: ['clubs'],
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private clubService: ClubService) {
		super(meta, paramDef, async (ps, me) => {
			try {
				return await this.clubService.invitationStatusFor(me, ps.userId);
			} catch (e) {
				return toApiError(e);
			}
		});
