...
export const paramDef = {
	type: 'object',
	properties: { channelId: { type: 'string', format: 'misskey:id' }, userId: { type: 'string', format: 'misskey:id' }, role: { type: 'string', enum: ['admin', 'member'] }, tags: { type: 'array', items: { type: 'string', maxLength: 32 }, maxItems: 12 }, remove: { type: 'boolean' } },
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private clubService: ClubService) {
		super(meta, paramDef, async (ps, me) => {
			...
			await this.clubService.updateMember(c, me, ps.userId, { role: ps.role, tags: ps.tags, remove: ps.remove });
			return { ok: true };
			} catch (e) {
				return toApiError(e);
			}
