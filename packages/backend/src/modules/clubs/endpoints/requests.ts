...
export const meta = {
	...
	kind: 'read:channels',
	res: { type: 'array', optional: false, nullable: false, items: { type: 'object' } },
	errors: clubErrors,
} as const;

export const paramDef = {
	type: 'object',
	properties: { channelId: { type: 'string', format: 'misskey:id' }, status: { type: 'string', enum: ['pending', 'approved', 'declined'], default: 'pending' } },
	required: ['channelId'],
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private clubService: ClubService) {
		super(meta, paramDef, async (ps, me) => {
			...
			return await this.clubService.joinRequests(c, me, ps.status);
			} catch (e) {
				return toApiError(e);
			}
		});
