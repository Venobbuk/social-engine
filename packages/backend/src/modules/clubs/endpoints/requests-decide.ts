...
export const paramDef = {
	type: 'object',
	properties: { channelId: { type: 'string', format: 'misskey:id' }, requestId: { type: 'string', format: 'misskey:id' }, approve: { type: 'boolean' } },
	required: ['channelId', 'requestId', 'approve'],
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private clubService: ClubService) {
		super(meta, paramDef, async (ps, me) => {
			...
			await this.clubService.decide(c, me, ps.requestId, ps.approve);
			return { ok: true };
			} catch (e) {
				return toApiError(e);
			}
