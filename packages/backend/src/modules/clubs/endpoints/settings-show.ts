...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private clubService: ClubService) {
		super(meta, paramDef, async (ps, me) => {
			...
			// SEC-CLUB-TAGS-V1 (2026-09-21, permission-sweep hole 8): review-batch2 #10 closed membersCount/followersCount
			// behind hasAccess and left THIS line open, so a private club still published its per-tag sizes ("Coaches 3",
			// "Committee 5") to an outsider — the same audience fact by another name. Same gate, same answer: nothing.
			const tagsVisible = hasAccess ? s.tags.filter(t => isAdmin || t.visibility === 'all').sort((a, b) => a.order - b.order).map(t => ({ id: t.id, name: t.name, visibility: t.visibility, order: t.order, count: Object.keys(t.members).length })) : [];
			return { refCode: s.refCode, accessToken: isAdmin || isMember || tokenOk ? s.accessToken : null, venues: await this.clubService.venues(s), tags: tagsVisible, awards: s.awards, myPinned: !!me && (await this.clubService.pinnedChannelIds(me.id, [c.id])).has(c.id) /* NUKE-CLUB-PIN-V1: native channel_favorite */, myPaused: !!(st && st.pausedAt), pinnedNoteIds: c.pinnedNoteIds, hasAccess, /* CLUB-INVITE-V1: an invited player may see a private club to decide */ visibility: s.visibility, gateType: s.gateType, createMeetPermission: s.createMeetPermission, memberGated: !!s.memberGated /* MEETS-FIXES-V1 */, allowOutsideLinks: s.allowOutsideLinks !== false /* CLUB-POSTS-LINKS-V1 */, handle: s.visibility === 'public' || isAdmin || isMember || tokenOk ? (s.handle ?? null) : null /* CLUB-HANDLE-V1: a private club's handle is its members' (G15.5) */, sport: s.sport, level: s.level, adminIds: isAdmin ? s.adminIds : [], venueIds: s.venueIds, paymentInfo: s.visibility === 'public' || isAdmin || isMember || tokenOk ? s.paymentInfo : null /* CLUB-PRIVATE-V1 (W1): a private club's payment details are its members' */, enableForum: s.enableForum, enableChat: s.enableChat, isAdmin, isOwner: !!me && c.userId === me.id, isMember, myRequest: me ? await this.clubService.myRequest(c.id, me.id) : null, myClaim: me && !c.userId ? await this.clubService.myClaim(c.id, me.id) : null, myInvitation, ...tiers }; // CLUB-CLAIM-VERIFY-V1: myClaim // CLUB-INVITE-V1: myInvitation
			} catch (e) {
				return toApiError(e);
			}
		});
