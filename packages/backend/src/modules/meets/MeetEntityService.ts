...
export class MeetEntityService {
	...
	public async pack(
		...
		return {
			...
			// MEET-EXTRAS-V1: listing link / promotion receipt / scoring rules, one object so the schema grows by one line
			extras: {
				externalUrl: meet.externalUrl ?? null,
				contactInfo: meet.contactInfo ?? null,
				promotedAt: meet.promotedAt ? new Date(meet.promotedAt).toISOString() : null,
				promotedReach: meet.promotedReach ?? 0,
				scoringType: meet.scoringType ?? 'GAME',
				standingsMode: meet.standingsMode ?? 'winLoss',
				tiebreakers: meet.tiebreakers ?? [],
				forfeitScore: meet.forfeitScore ?? 11,
				winPoints: meet.winPoints ?? 1,
				lossPoints: meet.lossPoints ?? 0,
				drawPoints: meet.drawPoints ?? 0,
				tbWinPoints: meet.tbWinPoints ?? 1,
	...
	public async packMatch(m: MiMeetMatch, meet: MiMeet, me?: MiUser | null, opts: { eligibility?: boolean } = {}): Promise<Packed<'MeetMatch'>> {
		...
		return {
			...
			canUpdateScore: canUpdateScore && m.duprStatus !== 'submitted' && m.duprStatus !== 'queued' && (await this.meetMatchService.casualLockReason(meet, m)) == null,
			canManage: isHost,
