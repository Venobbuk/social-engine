...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			const base = { gate, windowHours: PROMOTE_WINDOW_HOURS, radiusKm: PROMOTE_RADIUS_KM, promotedAt: meet.promotedAt ? new Date(meet.promotedAt).toISOString() : null };
			if (ps.preview) {
				const a = gate === 'ok' ? await promoteAudience(this.db, meet) : { userIds: [], followers: 0, nearby: 0 };
				return { ...base, reach: a.userIds.length, followers: a.followers, nearby: a.nearby, sent: false };
			}
			...
			const audience = await promoteAudience(this.db, meet);
			...
			return { ...base, gate: 'ok', reach: userIds.length, followers: audience.followers, nearby: audience.nearby, sent: true, promotedAt: now.toISOString() };
		});
	}
}
