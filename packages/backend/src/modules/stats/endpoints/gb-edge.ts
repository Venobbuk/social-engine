...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			// SEC-ANON-FIELDS-V1 (2026-09-20) → SEC-CHEM-V2 (2026-09-24): a negative partner-chemistry score is private to
			// the TWO players. edgeOf applies it (GbRating.chemFor) BEFORE the sort and the top-8 cut — V1 filtered here,
			// after the cut — and now also lets the partner see the one pair they are in, which V1 hid from them.
			const e = await edgeOf(this.db, ps.userId, ps.sport, me?.id ?? null) as Record<string, any>;
