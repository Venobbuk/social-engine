	...
	public async pack<S extends 'MeDetailed' | 'UserDetailedNotMe' | 'UserDetailed' | 'UserLite' = 'UserLite'>(
		...
		const packed = {
			...
			...(isDetailed ? {
				...
				// PE-PLACEMENTS-VISIBLE-V1 (press-every 2026-09-27): only competitions the viewer may open (assertVisible's rule) — a private one leaked its name and its row led to a 400
				placements: this.usersRepository.query('SELECT a."competitionId", c.name AS "competitionName", a.type, a.name, e.name AS "entryName", a."awardedAt" FROM competition_award a JOIN competition c ON c.id = a."competitionId" LEFT JOIN competition_entry e ON e.id = a."entryId" WHERE $1 = ANY(a."userIds") AND a.enabled = true AND (c.visibility = \'public\' OR c."hostId" = $2 OR $2 = ANY(a."userIds") OR EXISTS (SELECT 1 FROM competition_entry e2 WHERE e2."competitionId" = c.id AND $2 = ANY(e2."userIds"))) ORDER BY a."awardedAt" DESC NULLS LAST LIMIT 50', [user.id, me?.id ?? null]).then((rows: { competitionId: string; competitionName: string; type: string; name: string; entryName: string | null; awardedAt: Date | null }[]) => rows.map((r) => ({ competitionId: r.competitionId, competitionName: r.competitionName, type: r.type, name: r.name, entryName: r.entryName ?? '', awardedAt: r.awardedAt ? new Date(r.awardedAt).toISOString() : null }))).catch(() => []),
			} : {}),
