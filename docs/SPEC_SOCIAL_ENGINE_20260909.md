...
| Community model (operator, 2026-09-10: "in hkpl a club is a real club; in Reclub a club is a person opening a page — that is the real gap") | Two kinds of community, both Misskey channels, distinguished by a badge: **Verified clubs** — mirrored from hkpl (196), badge "HKPL club", cannot be self-created, managers = hkpl ClubManager, link back to teams/standings; **Open groups** — any member creates one in a tap (Reclub's "club": a person, a court, a coach), no badge. Any member may host a meet under their own name or an open group; verified clubs host as the club. Host trust for individuals comes from their DUPR + reliability score, which Reclub cannot give. | — |
...
1b. **SSO accounts skip the first-login wizard** — `/sso` page marks the account-setup wizard done before redirecting (the host already provided name/avatar); also copy the host avatar into the account on first sign-in (drive upload from `claims.avatar`). Observed 2026-09-10: the walk landed on /channels logged in but with the stock "Profile setup" prompt.
9b. **Visitor landing page (client patch, small)** — remove "Powered by Misskey" from the welcome page (AGPL source offer stays on /about via `repositoryUrl` → our fork), hide "Look for another instance" and the federation warning when `federation: none`, replace the invite-only warning with one plain sentence, hide the users/notes counters. Configurable parts already applied 2026-09-10: HKPL banner as background, white HKPL logo, HKPL light/dark default themes, repo + feedback links.
