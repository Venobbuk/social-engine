...
const OUT = __dirname + '/fix-S4.webkit.w45c.json';
...
(async () => {
  ...
  try {
    ...
    // W4 range calendar
    {
      ...
      await b.page.waitForSelector('.dv-filter', { timeout: 90000 }).catch(() => null); await sleep(1500); await tapSel(b.page, '.dv-filter', null); await b.page.waitForSelector('.dv-datechip', { timeout: 60000 }).catch(() => null); await sleep(800);
      await tapSel(b.page, '.dv-datechip', null, 'C-filter-dates.01', 'Dates chip'); await b.page.waitForSelector('.rc-d.is-tap', { timeout: 15000 }).catch(() => null); await sleep(600);
