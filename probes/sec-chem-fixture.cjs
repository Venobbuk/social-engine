...
/** The chemistry of pair a+b as a given viewer's doors must compute it: rows of a with partner b, NOT skipped, whose
 *  meet match still exists in a live meet the viewer may see ('' = anonymous → public meets only). */
function guardedPair(sql, a, b, viewer) {
	...
	return one(sql, `SELECT row_to_json(t) FROM (SELECT count(*)::int n, coalesce(sum(CASE WHEN l.won THEN 1 ELSE 0 END),0)::int w, coalesce(sum(l.expected),0)::float e
		...
		WHERE l.source = 'meet' AND l."userId" = '${a}' AND l."partnerId" = '${b}' AND l.sport = 'pickleball' AND NOT l.skipped
		  AND m.status <> 'cancelled'
		  AND (m.visibility = 'public' OR m."hostId" = '${v}' OR EXISTS (SELECT 1 FROM meet_participant vp WHERE vp."meetId" = m.id AND vp."userId" = '${v}'))) t;`);
}
...
async function build(P, o) {
	...
	fx.g1 = await game('chem g1', A, [A, B], [C, E], [[5, 11]], 5, true);
	fx.g2 = await game('chem g2', A, [A, B], [C, E], [[6, 11]], 4, true);
	fx.g3 = await game('chem g3', C, [C, D], [A, E], [[11, 5]], 3, true);
	fx.g4 = await game('chem g4', C, [C, D], [A, E], [[11, 6]], 2, true);
	...
	// ── the engine's OWN rating pass: wait for gb_rating_log rows of all five matches (4 rows each, none skipped) ──
	const matchIds = [fx.g1, fx.g2, fx.g3, fx.g4, fx.g5].map((g) => g.matchId);
	...
	log('INFO engine rating pass wrote ' + rated + ' / 20 gb_rating_log rows for the five fixture matches');
	if (rated < 20) throw new Error('the engine did not rate the fixture within 300 s (' + rated + '/20 rows) — refusing to measure an unrated fixture');
	...
	must(fx.pos.outsider && fx.pos.outsider.n === 2 && fx.edges.pos > 0, 'pair C+D has 2 PUBLIC live rated matches and a POSITIVE edge (the public control)', fx.pos);
	...
	must(fx.negAE.member && fx.negAE.member.n === 2 && edgeOfRec(fx.negAE.member) < 0, 'pair A+E is negative too (A\'s Edge has TWO negative partners, each private to its own pair)', fx.negAE);
	const vis = sql(`SELECT string_agg(id || ':' || visibility || ':' || status, ',' ORDER BY "startAt") FROM meet WHERE id IN ('${fx.g1.meetId}','${fx.g2.meetId}','${fx.g3.meetId}','${fx.g4.meetId}','${fx.g5.meetId}');`);
	must(vis.split(',').filter((x) => /:public:active$/.test(x)).length === 4 && vis.split(',').filter((x) => /:private:active$/.test(x)).length === 1, 'meets: four public + one private, all active', vis);
...
module.exports = { build, teardown, guardedPair, edgeOfRec, sqlOn };
