...
const rows = [
  ...
  row('A-short-links.01', ['S1', 'S2'], ['U14a', 'U14c', 'U14d'], 'REUSED meets/show {referenceCode}; NEW pages/link resolver', 'app half L6 (pages/link?m= → meet); the nginx /m/ rewrite is STAGED by the orchestrator, so https://uat.gripbat.com/m/<code> still answers 200 (the app shell)'),
  row('B-link-club.02', ['H3'], ['U14b', 'U14d'], 'EXTENDED clubs/by-code {handle}', 'app half L6 (pages/link?club=); the nginx /clubs/@ rewrite is STAGED'),
...
const out = {
  ...
  running: { engine: 'image rev b862cec790 (carries 739d7fc0b7), web + web-uat equal', app: '70fe204 (13:26 deploy)' },
