// fix-S4 STAFF probe, PC edition (WebKit banner check); fixtures cleaned in finally; the box sweep runs after (lane runs it).
...
const getNativeToken = async (key) => JSON.parse(execFileSync('ssh', ['kaka', 'cat /root/gb-native-' + key + '-uat.token'], { stdio: ['ignore', 'pipe', 'ignore'] }).toString());
...
const DIR = __dirname;
const OUT = DIR + '/fix-S4-staff.pc.json';
const SHOTS = DIR + '/fix-S4-staff-shots'; fs.mkdirSync(SHOTS, { recursive: true });
...
const hkdb = (q) => execFileSync('ssh', ['kaka', 'docker exec -i hkpl-docker-hkpl-db-1 sh -c \'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -qtA\''], { input: q, stdio: ['pipe', 'pipe', 'ignore'] }).toString().trim();
...
const code = () => execFileSync('ssh', ['kaka', 'bash /root/gen/fix-s4/sbxcode.sh'], { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('\n').pop();
...
(async () => {
  ...
  try {
    ...
    const bannerSeen = async (title, tag) => {
      const br = await webkit.launch(); const ctx0 = await br.newContext({ ...devices['iPhone 13'] }); const ctx = { addInitScript: (...x) => ctx0.addInitScript(...x), close: async () => { await ctx0.close().catch(() => null); await br.close().catch(() => null); } }; const page = await ctx0.newPage();   // a fresh WebKit per look (one process degraded on this PC)
      await ctx.addInitScript((t) => { try { localStorage.setItem('hkpl_lang_ok', JSON.stringify({ data: '1' })); localStorage.setItem('boyau_social_token', JSON.stringify({ data: t })); localStorage.setItem('boyau_onboarded', '1'); } catch (e) { /* */ } }, amy.token);   // UAT-PROBE-HIDE-V1: on UAT a [probe] item is shown only to a probe persona (amy, @hkpl-test.silkvo.com)
      const seenNet = []; const cons = []; page.on('console', (m) => { if (m.type() === 'error') cons.push(m.text().slice(0, 160)); }); page.on('pageerror', (e) => cons.push('pageerror ' + String(e.message).slice(0, 160))); page.on('response', async (r) => { if (/\/api\/announcements$/.test(r.url())) { let j = null; try { j = await r.json(); } catch (e) { /* */ } seenNet.push({ s: r.status(), req: (r.request().postData() || '').slice(0, 60), n: Array.isArray(j) ? j.length : null, raw: JSON.stringify(j).slice(0, 900) }); } });
      await page.goto(APP + '/app/pages/home/index?lang=en', { waitUntil: 'networkidle', timeout: 60000 }).catch(() => null);
      await page.waitForFunction((t) => [...document.querySelectorAll('.sb-bar')].some((e) => (e.innerText || '').includes(t)), title, { timeout: 60000 }).catch(() => null);
      const bars = await page.$$eval('.sb-bar', (e) => e.map((x) => x.innerText.replace(/\s+/g, ' ').slice(0, 120))).catch(() => []);
      const dom = await page.evaluate((t) => ({ sb: document.querySelectorAll('.sb-bar').length, textHas: document.body.innerText.includes(t), hidden: localStorage.getItem('gb_banner_hidden'), head: (document.querySelector('.sh-app-body') || document.body).innerHTML.slice(0, 200) }), title).catch((e) => ({ err: String(e).slice(0, 100) }));
      const f = SHOTS + '/' + tag + '.png'; await page.screenshot({ path: f, timeout: 20000 }).catch(() => null);
      await ctx.close(); return { bars, seen: bars.some((b) => b.includes(title)), shot: f.replace(DIR + '/', 'probes/'), net: seenNet, dom, cons: cons.slice(0, 6) };
    };
    ...
    chk('C-root-layout.04', 'S1 staff raises a banner (admin/announcements/create 200) → the public announcements door lists it → /app/ at 390 px (WebKit, iPhone 13, read by probe persona amy: UAT hides [probe] items from testers) shows it; staff clears it (isActive false) → gone from the door and the app', cA.s === 200 && Array.isArray(pub.j) && pub.j.some((a) => a.id === annId && a.display === 'banner') && on.seen && (uA.s === 204 || uA.s === 200) && Array.isArray(pub2.j) && !pub2.j.some((a) => a.id === annId) && !off.seen, { create: cA.s, update: uA.s, publicRaw: JSON.stringify(pub.j).slice(0, 800), onBars: on.bars, onDom: on.dom, onCons: on.cons, onNet: on.net, offBars: off.bars, offNet: off.net, shots: [on.shot, off.shot] });
