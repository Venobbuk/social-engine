#!/usr/bin/env node
// probes/meet-v4-claim.cjs — local driver: copies meet-v4-claim.remote.cjs to kaka, runs it there against the live
...
try {
  execFileSync('scp', ['-q', '-o', 'BatchMode=yes', REMOTE_SRC, 'root@kaka.silkvo.com:/root/social-engine/probes/meet-v4-claim.remote.cjs'], { stdio: 'ignore' });
  const out = execFileSync('ssh', ['-o', 'BatchMode=yes', '-o', 'ConnectTimeout=15', 'root@kaka.silkvo.com', 'cd /root/social-engine && node probes/meet-v4-claim.remote.cjs'], { encoding: 'utf8', timeout: 240000, stdio: ['ignore', 'pipe', 'ignore'] })
